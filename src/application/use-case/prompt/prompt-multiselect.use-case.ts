...
import type { TPromptMultiselectArgumentsType } from "./type";

import { mapSelectedValuesToLabelsFunction } from "./map-selected-values-to-labels.function";
import { renderCompletedMultiselectLineFunction } from "./render-completed-multiselect-line.function";
import { renderMultiselectFrameFunction } from "./render-multiselect-frame.function";
...
export class PromptMultiselectUseCase {
	...
	public async execute(input: TPromptMultiselectArgumentsType): Promise<null | ReadonlyArray<string>> {
				...
				options.length === 0
					...
					: ((): string => {
							...
							return (
								...
								renderMultiselectFrameFunction({
									cursorIndex,
									errorMessage,
		...
		return await new Promise<null | ReadonlyArray<string>>((resolve: (value: null | ReadonlyArray<string>) => void) => {
			...
			const finish = (result: null | ReadonlyArray<string>): void => {
				...
				const completedLine: string = renderCompletedMultiselectLineFunction({
					isUnicodeSupported: this.OUTPUT_PORT.IS_UNICODE_SUPPORTED,
					message: input.message,
					promptStylePort: this.PROMPT_STYLE_PORT,
					selectedLabels,
